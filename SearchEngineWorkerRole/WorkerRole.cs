        ...
        internal class PageWorker : ThreadWorker
        {
            ...
            private void ParseHtml(string htmlUrl)
            {
                ...
                try
                {
                    ...
                    {
                        throw new Exception("Broken HTML DOM");
                    }
                    ...
                    foreach (string partial in title.Split(' '))
                    {
                        if (r.IsMatch(partial) && !String.IsNullOrWhiteSpace(partial))
                            InsertToTable(new TitlePartialEntity(partial, htmlUrl, title, date));
                    }
                ...
                {
                    lastTenError.Enqueue("Error: " + e.Message + " on url " + htmlUrl);
                }
            }

            private void InsertToTable(TitlePartialEntity titlePage)
            {
